...
//$status = $_POST['status'];
        $id = uniqid();
        $q3 = mysqli_query($con, "INSERT INTO quiz VALUES  ('$id' , '$name' , '$sahi' , '$wrong' , '$total' , '$time' , '$desc' , '$tag', NOW() , '$status' )");
...
//        $refnum = @$_GET['ref'];
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $qns = $_POST['qns' . $i];
            $refnum = $_POST['ref' . $i];
            $q3 = mysqli_query($con, "INSERT INTO questions VALUES  ('$eid','$qid','$qns' , '$ch' , '$i','D','$refnum')");
            ...
            $qe = mysqli_query($con, "INSERT INTO options VALUES  ('$qid','$e','$oeid')") or die('Error65');
            $f = $_POST['ans' . $i];
...
//**************** AWNSER*************************
            ...
            $qans = mysqli_query($con, "INSERT INTO answer VALUES  ('$qid','$ansid','$f')");
...
//        ************************************************************************************************************
//        ***************************add insert all questions and awnsers to user start ******************************
//        ***********************************************************************************************************

//        $optiontxt = mysqli_query($con, "SELECT option FROM options WHERE optionid='$ans'") or die('Error');
        $row = mysqli_fetch_array($con, "SELECT option FROM options WHERE optionid='$ans'");

//        $result = $con->query($optiontxt);  // or mysqli_query($con, $tourquery);
//
//        $tourresult = $result->fetch_array()[1] ?? '';
// OR
//        $tourresult = $result->fetch_array()['roomprice'] ?? '';



        $q = mysqli_query($con, "INSERT INTO summary VALUES('$email','$eid' ,'$qid','$qns','$ans','$row')") or die('Error');

        //  ************************************************************************************************************
//        ***************************add insert all questions and awnsers to user end ******************************
//        ***********************************************************************************************************
        ...
        //add insert all questions and awnsers to user
